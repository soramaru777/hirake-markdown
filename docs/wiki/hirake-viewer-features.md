...
ファイルごとのスクロール位置を記憶し、引数なし起動では前回のタブを復元する。キャンバス・構造クエリ・リンク検出・指紋・統計といった**仮想タブも復元対象**で、復元は**ウィンドウの枚数ごと**行われる。詳細は [[hirake-multi-window]]、名前を付けて保存する方は [[hirake-workspaces]]。
