...
### 外部への受け渡し口としても使える（2026-08-17 追記）

この機能は Hirake 同士のリンク用に作ったものだが、**パスと行番号が両方入っているため、そのまま外部へ渡せる**。読んでいる箇所をコピーして AI に貼れば、ファイルと該当行を特定して直させられる。Hirake 自身は書き換えないという方針（→ [[hirake-editing-boundary]]）のもとで、**既存機能だけで成立している唯一の編集経路**になっている。
