...
| 🕸️ | **ナレッジグラフ** | 文書間のリンクを解析して関係を地図のように可視化（Ctrl+G）。「この文書を参照している文書」のバックリンク一覧も常設表示 |
...
- **無限キャンバス・モード（Ctrl+Shift+C）**: フォルダ配下の全文書をズーム可能な無限キャンバスへ空間配置。ズームアウトでリンクグラフ、ズームインでサムネイル付きカードになり、ドラッグ配置はピン留めとして記憶されます
