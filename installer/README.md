...
> **リリースのタグは、`main` か `develop` にマージ済みのコミットに打ってください。**
> どちらにも含まれないコミットに `v*` タグを打つと、`build` ジョブが失敗して
> リリースは作られません（未マージのコードで配布物ができるのを防ぐため・ISSUE #52）。
>
> この検証は**操作ミスを防ぐためのもの**です。ワークフローはタグが指すコミット側の
> 定義で動くため、検証を消したコミットにタグを打てば素通りします。権限として
> 絞るには、リポジトリのルールセットで `v*` タグの作成を制限してください。
